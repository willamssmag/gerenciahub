...
for indice, projeto in enumerate(projetos, start=1):
  ...
  function fillMetadataForm(project = {}) {
    ...
    form.id.readOnly = Boolean(editingId);
    form.id.title = editingId ? "O identificador é fixo para atualizar apenas esta pasta." : "";
  ...
  function openMetadata(mode) {
    ...
    elements.metadataIntro.textContent = publishing
      ? "Somente a pasta deste projeto será enviada ao GitHub. A página inicial e os demais projetos não serão alterados."
  ...
  function collectProject() {
    ...
    return storage.normalizeProject({
      ...
      managed: true,
      live: `/visualizador/project.html?id=${encodeURIComponent(id)}`
    });
  }
  ...
  async function saveProject(event) {
    ...
    try {
      if (saveMode === "local") {
        ...
        editingId = saved.id;
        history.replaceState(null, "", `/visualizador/?edit=${encodeURIComponent(saved.id)}`);
      ...
      const response = await fetch("/api/github-project", {
        method: currentProject?.folder ? "PUT" : "POST",
        headers: { "Content-Type": "application/json", "X-Admin-Password": password },
        body: JSON.stringify({ project, code: elements.editor.value, originalId: editingId || "" })
      });
      ...
      history.replaceState(null, "", `/visualizador/?edit=${encodeURIComponent(editingId)}`);
      elements.pageTitle.textContent = `Editando ${result.project.title}`;
      setStatus("Commit enviado ao GitHub", "success");
      elements.metadataDialog.close();
      showToast("A pasta deste projeto foi atualizada no GitHub. Os demais projetos não foram modificados.", "success");
    } catch (error) {
  ...
  async function loadRemoteProjects() {
    try {
      const response = await fetch(`/api/portfolio-projects?v=${Date.now()}`, { cache: "no-store" });
  ...
  async function initializeProject() {
    ...
    const projects = storage.mergeProjects(storage.getProjects(), remote);
    ...
    } else if (currentProject?.source) {
      try {
        const response = await fetch(currentProject.source, { cache: "no-store" });
        if (response.ok) {
          elements.editor.value = await response.text();
          setStatus("Código da pasta do projeto carregado", "success");
        }
