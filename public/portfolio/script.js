(() => {
  ...
  let allProjects = storage.mergeProjects(defaultProjects, storage.getProjects(), publishedProjects);
  ...
  function createProjectCard(project) {
    ...
    if (project.managed) {
      editLink.href = `/visualizador/?edit=${encodeURIComponent(project.id)}`;
  ...
  async function loadPublishedProjects() {
    try {
      let response = await fetch(`/api/portfolio-projects?v=${Date.now()}`, { cache: "no-store" });
      if (!response.ok) {
        response = await fetch(`/projects.json?v=${Date.now()}`, { cache: "no-store" });
      }
      if (!response.ok) return;
      const projects = await response.json();
      if (!Array.isArray(projects)) return;
      publishedProjects = projects;
      allProjects = storage.mergeProjects(defaultProjects, storage.getProjects(), publishedProjects);
      renderAll();
    } catch {
  ...
  elements.openAddProject.addEventListener("click", () => { location.href = "/visualizador/"; });
