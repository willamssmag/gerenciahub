...
## Novidade da versão 2.1.2

- Barra de pesquisa permanente dentro do repositório.
- Busca por nome, extensão ou caminho completo em toda a branch selecionada.
- Resultados em tempo real com caminho completo do arquivo.
- Abertura direta do arquivo ou navegação para a pasta encontrada.
- Navegação por teclado com `↑`, `↓`, `Enter` e `Esc`.
- Cache local do índice para respostas mais rápidas.
- O atalho `Ctrl + P` ou `Cmd + P` continua disponível para pesquisa em janela.

### Usar a barra de pesquisa

1. Abra um repositório e selecione a branch desejada.
2. Digite na barra **Pesquisar arquivo ou pasta em todo o repositório**.
3. Pesquise por exemplos como `index.html`, `.json`, `src/` ou parte de um nome.
4. Clique no resultado ou use as setas e `Enter`.

A busca considera toda a branch selecionada, não apenas o diretório atualmente aberto.
...
Use a barra permanente abaixo do cabeçalho do repositório para pesquisar em toda a branch. A pesquisa rápida em janela também continua disponível pelo botão **⌕ Buscar** ou pelo atalho `Ctrl + P`. Use as setas e `Enter` para navegar.
